...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="B站知识库采集器")
    subparsers = parser.add_subparsers(dest="command", required=True)

    run_parser = subparsers.add_parser("run", help="执行主题搜索采集")
    run_parser.add_argument("--pack", type=str, default=DEFAULT_PACK_SLUG)
    ...
    resume_parser.add_argument("--pack", type=str, default=DEFAULT_PACK_SLUG)
    ...
    export_parser.add_argument("--pack", type=str, default=DEFAULT_PACK_SLUG)
    export_parser.add_argument("--output-dir", type=str, default=None)

    validate_parser = subparsers.add_parser("validate", help="校验环境和配置")
    validate_parser.add_argument("--pack", type=str, default=DEFAULT_PACK_SLUG)
    ...
    space_parser.add_argument("--pack", type=str, default=DEFAULT_PACK_SLUG)
    ...
    retry_space_parser.add_argument("--pack", type=str, default=DEFAULT_PACK_SLUG)
...
def _export_catalog_with_guard(records: list, output_dir: Path, pack_slug: str, snapshot_meta: dict | None = None) -> dict:
    pack = get_pack(pack_slug)
    try:
        return export_catalog(records, output_dir, pack=pack, snapshot_meta=snapshot_meta)
...
def run_crawler(args: argparse.Namespace) -> int:
    ...
    result = run_topic_pipeline(
        config=config,
        pack=pack,
        keyword_overrides=args.keyword,
    ...
    snapshot_meta = {
        "mode": "snapshot",
        "status": "succeeded" if summary["counters"]["failed_tasks"] == 0 else "partial",
        "job_id": None,
        ...
        "last_refreshed_at": summary["finished_at"],
        "last_error": None,
        "output_dir": str(config.output_dir),
    }
    export_paths = _export_catalog_with_guard(result.records, config.output_dir, pack.slug, snapshot_meta=snapshot_meta)
    summary["status"] = "succeeded" if summary["counters"]["failed_tasks"] == 0 else "partial"
    summary["export_paths"] = export_paths
    CatalogStore(config).finalize_run(summary)
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    has_export_error = "error" in export_paths
    return 0 if summary["counters"]["failed_tasks"] == 0 and not has_export_error else 1
...
def export_existing(args: argparse.Namespace) -> int:
    pack = get_pack(args.pack)
    config = load_config(output_dir=args.output_dir, pack_slug=pack.slug)
    config.ensure_directories()
    store = CatalogStore(config)
    records = list(store.load_catalog().values())
    if not records:
        print(json.dumps({"message": "未找到现有 JSONL 数据。"}, ensure_ascii=False, indent=2))
        return 1
    export_paths = _export_catalog_with_guard(records, config.output_dir, pack.slug)
    print(json.dumps(export_paths, ensure_ascii=False, indent=2))
    return 0 if "error" not in export_paths else 1


def export_space_checkpoint(output_dir: Path, uploader_name: str, pack_slug: str) -> dict:
    ...
    all_exports = _export_catalog_with_guard(records, output_dir, pack_slug)
    tutorial_dir = output_dir / "tutorial_only"
    tutorial_dir.mkdir(parents=True, exist_ok=True)
    tutorial_exports = _export_catalog_with_guard(tutorial_records, tutorial_dir, pack_slug)
    return {
...
def validate_environment(args: argparse.Namespace) -> int:
    get_pack(args.pack)
    config = load_config(output_dir=args.output_dir, pack_slug=args.pack)
    config.ensure_directories()
    checks = {
        "pack_slug": config.pack_slug,
        "available_packs": [pack.slug for pack in list_packs()],
        ...
        "pages": config.pages,
        "catalog_exists": config.catalog_jsonl_path.exists() or config.legacy_catalog_jsonl_path.exists(),
...
def run_space_crawler(args: argparse.Namespace) -> int:
    ...
    export_summary = export_space_checkpoint(config.output_dir, args.uploader_name, pack.slug)
...
def retry_space_failed(args: argparse.Namespace) -> int:
    ...
    if checkpoint_path.exists():
        before_records = len(
            [line for line in checkpoint_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        )
